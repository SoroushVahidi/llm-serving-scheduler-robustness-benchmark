...
def test_select_stride_windows_basic_shape():
    ...
    windows, report = select_stride_windows(
        lambda: records, window_size=3, n_windows=4, offset_valid_rows=10
    )
...
def test_select_stride_windows_drops_invalid_rows():
    ...
    _, report = select_stride_windows(lambda: records, window_size=1, n_windows=1, offset_valid_rows=0)
...
def test_select_stride_windows_raises_when_not_enough_rows():
    ...
    with pytest.raises(ValueError):
        select_stride_windows(lambda: records, window_size=200, n_windows=10, offset_valid_rows=0)


def test_select_stride_windows_deterministic_for_fixed_seed():
    records = _burstgpt_records(50)
    w1, r1 = select_stride_windows(
        lambda: records, window_size=3, n_windows=4, offset_valid_rows=10, seed=42
    )
...
def test_select_stride_windows_different_seeds_can_differ():
    ...
    _, r1 = select_stride_windows(
        lambda: records, window_size=3, n_windows=4, offset_valid_rows=10, seed=1
    )
    _, r2 = select_stride_windows(
        lambda: records, window_size=3, n_windows=4, offset_valid_rows=10, seed=999
    )
