...
\S\ref{sec:threats}). The simulator-predicted Azure/BurstGPT reversal did
not reproduce on real vLLM/GPU execution, exposing a fidelity limitation
specific to that selected comparison; the paired stable-control comparison
did retain its qualitative ordering, consistent with, but not a
confirmation of, the simulator's broader findings. Read together, the two
outcomes illustrate the same case-dependence documented throughout
\S\ref{sec:results} -- which comparative conclusions transfer and which do
not is case-dependent rather than uniform -- now observed at the
simulator-to-hardware boundary rather than across workload source,
operating region, or metric, without themselves adding statistical weight
to those earlier findings.
