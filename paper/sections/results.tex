...
\begin{itemize}
  ...
  \item \textbf{\texttt{WINDOW\_SIZE\_SENSITIVITY}.} Despite its name,
    this frozen contract label denotes the sample-complexity window-\emph{count}
    subsampling ladder (\S\ref{sec:sample-complexity}: how the
    ranking-portability estimate varies as fewer of the 40 windows per
    source are used), not the number of requests per window; not
    separately recomputed here. The number of requests per window (window
    \emph{size} in the sense of \S\ref{sec:window-size}) has not been
    swept and is a disclosed open item (\S\ref{sec:threats}), unrelated to
    this frozen contract item despite the shared name.
