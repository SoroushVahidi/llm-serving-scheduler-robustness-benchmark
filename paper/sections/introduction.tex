...
does not claim that any scheduler in the panel wins in general. RQ6
(\S\ref{sec:real-system}) tests whether the strongest identified simulated
reversal reproduces on real vLLM/GPU execution: the selected physical
validation did not reproduce that reversal, while a paired stable-control
comparison retained the same qualitative ordering on physical hardware --
further evidence that comparative scheduler conclusions are conditional
