...
Large language model (LLM) serving systems are high-performance,
GPU-resident computing systems whose throughput and latency are governed
as much by request scheduling and resource-management policy as by raw
accelerator capability. These systems have grown increasingly
sophisticated in how they schedule and allocate GPU resources to
concurrent requests. Beyond simple first-come-first-served execution,
modern serving stacks employ continuous batching and paged key-value (KV)
cache management
...
from combining these two observations. The typical evaluation question in
...
Consistent with the scientific state of the project at the time of writing
(\S\ref{sec:study-design}), this manuscript does not claim that scheduler
rankings are stable, that they are unstable, that any reversal exists, or
that any scheduler in the panel wins. Sections~\ref{sec:results}
through~\ref{sec:real-system} state the exact, preregistered result that
each pending experiment will report, without asserting its direction in
advance.
