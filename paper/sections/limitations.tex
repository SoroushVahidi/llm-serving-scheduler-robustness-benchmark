...
\paragraph{Window-size sensitivity.} The Pilot-V2 corpus fixes window size
...
top-ranked policy agrees in only $31.9\%$ of conditions (disagreeing in the
majority, $68.1\%$). This is a scope
...
check found $20$ of $72$ ($28\%$) non-trivial post-campaign
SLO-sensitivity comparisons sensitive to the SLO-definition choice (the
artifact's persistence matrix has $108$ total mathematical comparisons,
but $36$ of those are trivial self-comparisons against the primary
definition and are excluded from this denominator; $6$ direction changes, $10$ falling below the
...
statistical inference is drawn (\S\ref{sec:real-system-design}). Concretely,
Table~\ref{tab:rq6-results}'s confidence intervals capture
