...
LSSP evaluates a fixed panel of scheduling policies (\S\ref{sec:panel})
against a frozen corpus of workload windows drawn from independently
sourced traces (\S\ref{sec:sources}), across a calibrated grid of operating
regions (\S\ref{sec:calibration}) and a fixed set of primary and
completion-conditioned metrics (\S\ref{sec:metrics}). Every policy is run
against every window under every operating region, producing a matrix of
cells from which per-source, per-region, and per-metric scheduler rankings
are derived. Ranking-portability statistics (Kendall's tau-b, Spearman's
