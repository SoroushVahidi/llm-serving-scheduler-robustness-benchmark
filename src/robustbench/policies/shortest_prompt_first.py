"""SPF: Shortest Prompt First — prefer requests with fewest prompt tokens."""
...
class ShortestPromptFirstPolicy(BasePolicy):
    name = "shortest_prompt_first"

    def select_action(self, state: ObservableState) -> Action:
        admit: dict[int, list[int]] = {g.gpu_id: [] for g in state.gpu_states}

        queue = sorted(state.waiting_queue, key=lambda r: r.prompt_tokens)
