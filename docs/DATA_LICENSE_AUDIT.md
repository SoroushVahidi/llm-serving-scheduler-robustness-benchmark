...
| Bailian/Qwen anonymized traces | Apache-2.0 (repository `LICENSE`; README states Apache-2.0 for the dataset release) | Permits redistribution | No | Loader exists in `llm-serving-heuristic-evolution` (`workloads/bailian.py`) but was **not** ported into this repo's `workloads/external/adapters/` in this bootstrap (only the module-intervention-benchmark adapter-pattern sources were ported: BurstGPT, Azure, Mooncake, LMSYS). Porting Bailian to the `TraceAdapter` interface is next-session work, tracked in `docs/EXPERIMENT_CAMPAIGN_PLAN.md` Stage 0. |
