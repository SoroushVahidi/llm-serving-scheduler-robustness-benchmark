...
| `kv_occupancy_mean` / `kv_occupancy_max` | mean over GPUs of (KV tokens used ÷ `max_kv_tokens`) — **normalized KV demand relative to configured nominal KV capacity, not a hard-bounded fraction** (semantics corrected 2026-09-02, `docs/RANKING_PORTABILITY_PHASE12_TELEMETRY_SEMANTIC_AMENDMENT.md`: only KV-aware policies enforce `max_kv_tokens` at admission, so this value can legitimately exceed 1.0 for policies that do not) | `GPUState.step_kv_used` ÷ `GPUConfig.max_kv_tokens` (pre-existing history; denominator fixed by config) | per-step cross-GPU mean, then mean/max over steps | Yes | normalized demand, **[0, ∞)** — values > 1.0 are valid and mean demand exceeded nominal capacity | per-step, per-GPU |
